# 106.14.145.226:7003

**GPU**: RTX 5080 | **显存**: 16 GB | **空闲**: 15 GB
**内存**: 47 GB (空闲 29 GB)
**版本**: 0.19.3
**ComfyUI报告历史**: 有
**扫描时间**: 2026-06-26 08:08:50
**历史总数**: 16 | **成功**: 4

## 工作流列表

### 1. workflow_01.json
- **Prompt ID**: `293a5fd4-d57d-451c-a945-e2168f3f3848`
- **类型**: 文生视频
- **节点数**: 18
- **模型** (6):
  - Wan22_Bernini_HIGH_fp8_e4m3fn_scaled.safetensors
  - Wan22_Bernini_LOW_fp8_e4m3fn_scaled.safetensors
  - umt5-xxl-enc-bf16.safetensors
  - wan2.2_t2v_A14b_high_noise_lora_rank64_lightx2v_4step_1217.safetensors
  - wan2.2_t2v_A14b_low_noise_lora_rank64_lightx2v_4step_1217.safetensors
  - wan_2.1_vae.safetensors

### 2. workflow_02.json
- **Prompt ID**: `84f4c843-7ef4-4dbe-8425-11ffa22ba05c`
- **类型**: 文生视频
- **节点数**: 18
- **模型** (6):
  - Wan22_Bernini_HIGH_fp8_e4m3fn_scaled.safetensors
  - Wan22_Bernini_LOW_fp8_e4m3fn_scaled.safetensors
  - umt5-xxl-enc-bf16.safetensors
  - wan2.2_t2v_A14b_high_noise_lora_rank64_lightx2v_4step_1217.safetensors
  - wan2.2_t2v_A14b_low_noise_lora_rank64_lightx2v_4step_1217.safetensors
  - wan_2.1_vae.safetensors

### 3. workflow_03.json
- **Prompt ID**: `f4e6db27-bbae-48d1-83e5-77750f9a732e`
- **类型**: 其他/工具
- **节点数**: 1
- **模型** (0):

### 4. workflow_04.json
- **Prompt ID**: `4696b327-aa96-4dd4-879a-fdaee43e4316`
- **类型**: 其他/工具
- **节点数**: 1
- **模型** (0):
