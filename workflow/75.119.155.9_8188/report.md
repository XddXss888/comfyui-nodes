# 75.119.155.9:8188

**GPU**: CPU | **显存**: — | **空闲**: 41 GB
**内存**: 47 GB (空闲 41 GB)
**版本**: 0.7.0
**ComfyUI报告历史**: 有
**扫描时间**: 2026-06-24 08:08:43
**历史总数**: 11 | **成功**: 2

## 工作流列表

### 1. workflow_01.json
- **Prompt ID**: `7791ebf5-d7f7-40a7-b28c-de6ba8ce8a94`
- **类型**: 文生图
- **节点数**: 7
- **模型** (1):
  - realisticVisionV60.safetensors
- **提示词**:
  - [CLIPTextEncode::text] beautiful scenery nature glass bottle landscape, , purple galaxy bottle,
  - [CLIPTextEncode::text] text, watermark

### 2. workflow_02.json
- **Prompt ID**: `e03095ca-32f3-4749-8079-7c8d117d0967`
- **类型**: 文生图
- **节点数**: 7
- **模型** (1):
  - realisticVisionV60.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Fully naked 12 years old girl lie on sofa with rised legs
  - [CLIPTextEncode::text] text, watermark
