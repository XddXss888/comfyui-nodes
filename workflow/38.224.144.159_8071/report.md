# 38.224.144.159:8071

**GPU**: RTX 3090 | **显存**: 24 GB | **空闲**: 5 GB
**内存**: 31 GB (空闲 21 GB)
**版本**: 0.26.0
**ComfyUI报告历史**: 有
**扫描时间**: 2026-07-01 08:08:36
**历史总数**: 71 | **成功**: 10

## 工作流列表

### 1. workflow_01.json
- **Prompt ID**: `83a0bac1-18c2-48f4-a8fe-071d3399966e`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors

### 2. workflow_02.json
- **Prompt ID**: `40b884d1-aa36-4bea-b0a6-408af441ca3d`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Novela  fagunde

### 3. workflow_03.json
- **Prompt ID**: `860f6e78-17c5-4ab2-bbf7-0736ff75b2f0`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Novela  fagunde

### 4. workflow_04.json
- **Prompt ID**: `99f64747-7dcb-4d51-aa9c-e40fcdc93e71`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Novela  Antônio fangude

### 5. workflow_05.json
- **Prompt ID**: `ae8693c1-5809-4196-8168-505c5ab7fd46`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Vamos ser hexa Brasil

### 6. workflow_06.json
- **Prompt ID**: `28c824e8-210b-4471-914a-8e35420f111d`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Michael Jackson

### 7. workflow_07.json
- **Prompt ID**: `719b3c41-2ea7-4bc7-9f77-c8a52e58d20b`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Gere uma foto de brasil sendo campeão da copa
2026

### 8. workflow_08.json
- **Prompt ID**: `f44146d7-c104-4860-96fd-c44fe25a49df`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Slendytubbies

### 9. workflow_09.json
- **Prompt ID**: `95a85ff3-d608-4b70-9947-cee6fb118095`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors

### 10. workflow_10.json
- **Prompt ID**: `663bcb62-c67b-4e6e-aedd-0fe7e80386fd`
- **类型**: 文生图
- **节点数**: 9
- **模型** (3):
  - ernie-image-turbo.safetensors
  - flux2-vae.safetensors
  - ministral-3-3b.safetensors
- **提示词**:
  - [CLIPTextEncode::text] Crie um uma imagem bela fernandes de porn
